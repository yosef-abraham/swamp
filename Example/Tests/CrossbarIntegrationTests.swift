...
class CraTestSessionDelegate: TestSessionDelegate {
    private let craSecret: String
    init(craSecret: String) {
        self.craSecret = craSecret
    }
    
    override func swampSessionHandleChallenge(authMethod: String, extra: [String : AnyObject]) -> String {
...
class CrossbarIntegrationTestsSpec: QuickSpec {
    override func spec() {
        describe("the Open realm") {
            ...
            beforeEach {
                session = SwampSession(realm: "open-realm", transport: WebSocketSwampTransport(wsEndpoint: NSURL(string: "ws://localhost:8080/ws")!), authmethods: ["anonymous"])
            }
        ...
        describe("the Restrictive realm") {
            ...
            beforeEach {
                session = SwampSession(realm: "restrictive-realm", transport: WebSocketSwampTransport(wsEndpoint: NSURL(string: "ws://127.0.0.1:8080/ws")!), authmethods: ["wampcra"], authid: "homer")
            }
